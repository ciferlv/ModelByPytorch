...
def lda_sklearn():
    ...
    my_lda = LdaModel(text2bow, id2word=dictionary, num_topics=20, passes=20)

    print(my_lda.print_topics(num_topics=5, num_words=10))
...
def lda_scratch(topic_num, alpha, beta, passes):
    ...
    for i_pass in range(passes):
        ...
        for d, doc in enumerate(docs_idx_list):
            theta[d] = np.random.dirichlet(n_d_k[d] + np.ones((topic_num,)) * alpha, 1)
            for w_index, w in enumerate(doc):
                word = w
                topic = z[(d, w_index)]
                ...
                new_topic = np.random.multinomial(1, p_z_k / np.sum(p_z_k)).argmax()
    ...
    for k_i in range(topic_num):
        ...
        for idx, arg_index in enumerate(list(reversed(arg_list))):
            print("{}: {}".format(dictionary[arg_index], n_k_w[k_i, arg_index] / np.sum(n_k_w[k_i])), end="\t")
        print("\n")
